...
<?php
	...
	{
?>
	<h1 class="pageTitle">Add Association</h1>
	<h3 class="description">Photo associated with another Album?</h3>
  
  <form action="assocPic.php" method="post">
	<?php 
		...
		else
		{
			while ($pictureRow=$pictureResult->fetch_assoc())
			{
				while ( $row = $result->fetch_assoc() ) 
				{
					...
					$albumID=$row['albumID'];
					if($row['albumID']!=$pictureRow['albumID'])
	...
	if(isset($_POST['submit']))
	{
		...
		if($_POST['albums']=="null")
		...
		else
			{
				...
				require_once 'config.php';
				$mysqli = new mysqli( DB_HOST, DB_USER, DB_PASSWORD, DB_NAME );
				$result= $mysqli->query("INSERT INTO Associations VALUES ('$albumID', '$photoID')");
				print "<p> Added Successfully </p>";
			}
	}
	?>
	</body>
